...
function trainFacesNoise3(gpuid, options)

	method = 'faces-noise-5k';
...
function trainNet(options,  cachepath)
	...
	datasetfile = ['./data/faces/noise96_5k.mat'];
	if ~(exist(datasetfile, 'file'))
    		error('Dataset is not prepared!');
	end
	load(datasetfile, 'imagelist', 'edgeslist');
	...
	edgeslist = strcat('/mnt/pcie1/user/aayushb/Faces/', edgeslist);

	% load the network --
	solverpath = ['./data/net/faces/solver.prototxt'];
	initmodelpath = ['./data/net/faces/VGG16_fconv.caffemodel'];
	trainModel(options,trainfolder,imagelist, edgeslist,solverpath,initmodelpath);
end

function trainModel(options,trainfolder,imagelist,edgeslist,solverpath,initmodelpath)


	% Though this is not required -- 
	rand_ids = randperm(length(imagelist));
	imagelist = imagelist(rand_ids);
	edgeslist = edgeslist(rand_ids);
	...
	input_data = zeros(maxsize, maxsize, 3, options.labelsimbatch, 'single');
	input_index = zeros(3,options.labelsbatchsize, 'single');
	input_label = zeros(3,options.labelsbatchsize, 'single');
	solver.net.blobs('data').reshape([maxsize, maxsize, 3, options.labelsimbatch]);
	solver.net.blobs('pixels').reshape([3,options.labelsbatchsize]);
	solver.net.blobs('labels').reshape([3,options.labelsbatchsize]);
	...
	for epoch = 1:options.labelsepoch
   	 index = randperm(li);
   	 for i = 1:options.labelsimbatch:li
        	j = i+options.labelsimbatch-1;
        	if j > li
            	continue;
        	end

	        st = 1;
       		ed = options.labelssamplesize;
        	im = 0;
        	for k=i:j
            		ik = index(k);
	    		...
	    		im_data = edges_image_provider(options, edgeslist{ik}, flip);
	    		[sampled, label] = normals_label_provider(...
				options, imagelist{ik}, options.labelssamplesize, flip);
           		...
           		st = st + options.labelssamplesize;
            		ed = ed + options.labelssamplesize;
        	end
    	...
    	if mod(epoch, options.saveEpoch) == 0 && epoch < options.labelsepoch
