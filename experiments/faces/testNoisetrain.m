...
%
conv_cache = ['./cachedir/faces-noise/'];
if(~isdir(conv_cache))
        mkdir(conv_cache);
end

% initialize caffe
NET_FILE_PATH = ['./cachedir/faces-noise/TRAIN/'];
net_file     = [NET_FILE_PATH, '(04).caffemodel'];
...
load(['./data/faces/noise96.mat']);

% for each image in the img_set
for i = 1:length(edgeslist)

	display(['Image : ', edgeslist{i}]);
	...
	%
        save_file_name = [conv_cache, strrep(edgeslist{i}, '/', '-')];
