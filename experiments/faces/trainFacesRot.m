...
function trainFacesRot(gpuid, options)
...
function trainNet(options,  cachepath)
	...
	datasetfile = ['./data/faces/rot.mat'];
	if ~(exist(datasetfile, 'file'))
    		error('Dataset is not prepared!');
	end
	load(datasetfile, 'imagelist', 'rotlist');
	...
	rotlist = strcat('/mnt/pcie1/user/aayushb/Faces/', rotlist);

	% load the network --
	solverpath = ['./data/net/faces/solver.prototxt'];
	initmodelpath = ['./data/net/faces/VGG16_fconv.caffemodel'];
	trainModel(options,trainfolder,imagelist, rotlist,solverpath,initmodelpath);
end

function trainModel(options,trainfolder,imagelist,rotlist,solverpath,initmodelpath)


	% Though this is not required -- 
	rand_ids = randperm(length(imagelist));
	imagelist = imagelist(rand_ids);
	rotlist = rotlist(rand_ids);
	...
	for epoch = 1:options.labelsepoch
   	 ...
   	 for i = 1:options.labelsimbatch:li
        	...
        	for k=i:j
	    		...
	    		im_data = edges_image_provider(options, imagelist{ik}, flip);
	    		[sampled, label] = normals_label_provider(...
				options, rotlist{ik}, options.labelssamplesize, flip);
