...
% segmentation --
function generate_multi_outputs(REQ_PATHS, GPU)
	...
	for j = 1:size(nn_input,1)
		...
		for k = 1:nn_input(j,1)
			...
			kth_train_org_img = imread([TRAIN_IMG_ORG_PATH,...
                                '/', strrep(kth_train_img_id, 'edges_all', 'celebA_96_org')]);
			...
			nn_l_diff_data = (kth_train_org_img_l(:,:,1) - ...
						kth_train_img_l(:,:,1));
			...
			q_l_data = reshape(nn_l_diff_data, ...
					[size(nn_l_diff_data,1)*size(nn_l_diff_data,2),1]);
			...
			nn_pixel_ldata = nn_pixel_ldata + (Y.*q_l_data);
		...
		nn_pixel_ldata = nn_pixel_ldata./nn_pixel_lscore;
