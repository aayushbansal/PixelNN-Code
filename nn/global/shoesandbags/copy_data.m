...
function copy_data()
	...
	CACHENAME = 'catsanddogs-edges';

	VAL_DATA_PATH = ['./cachedir/', CACHENAME, '/feats/val/'];
	VAL_IMG_PATH = ['./cachedir/catsanddogs-edges/val/'];
	...
	TRAIN_IMG_PATH = ['./cachedir/catsanddogs-edges/train/'];
	TRAIN_IMG_ORG_PATH = ['./cachedir/catsanddogs-edges/train/'];
	...
	for i = 1:length(val_img_list)
		...
		for j = 1:NN_VIS
			...
			jth_train_org_img = imread([TRAIN_IMG_ORG_PATH,...
                        	        '/', strrep(jth_train_img_id,'edges', 'dataset-catsanddog')]);
