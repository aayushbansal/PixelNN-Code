...
for i = 1:length(val_img_list)
	...
	if(isLocked([DUMP_DATA_PATH, strrep(val_img_list{i}, '.mat', '')]))
		continue;
	end
	...
	save([DUMP_DATA_PATH, val_img_list{i}],...
				 'det_scores', 'img_list');
	unlock([DUMP_DATA_PATH, strrep(val_img_list{i}, '.mat', '')])
end
