...
function copy_data_multi()
	...
	TRAIN_IMG_ORG_PATH = ['./cachedir/faces-edges/train/'];
	...
	% --
	NN_VIS = 10;
	DUMP_DATA_PATH = ['./cachedir/', CACHENAME, '/hi-res-NN1_multi/'];
	...
	for i = 1:length(val_img_list)
		...
		for j = 1:NN_VIS
			...
			jth_train_org_img = imread([TRAIN_IMG_ORG_PATH,...
                        	        '/', strrep(jth_train_img_id,'edges_all', 'celebA_96_org')]);
			...
			imwrite(im_hr, [DUMP_DATA_PATH,  '/',....
        	        	   ith_image_name,'_', num2str(j, '%02d') '.png']);
