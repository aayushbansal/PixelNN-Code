...
function copy_data_normals()
	...
	CACHENAME = 'faces-normals';

	VAL_DATA_PATH = ['./cachedir/', CACHENAME, '/feats/val/'];
	VAL_IMG_PATH = ['./cachedir/faces-normals-val/'];
	...
	TRAIN_IMG_PATH = ['./cachedir/faces-normals/train/'];
	TRAIN_IMG_ORG_PATH = ['./cachedir/faces-normals/train/'];
	...
	for i = 1:length(val_img_list)
		...
		for j = 1:NN_VIS
			...
			jth_train_org_img = imread([TRAIN_IMG_ORG_PATH,...
                        	        '/', strrep(jth_train_img_id,'normals_all', 'celebA_96_org')]);
